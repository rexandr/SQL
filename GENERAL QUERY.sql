...
--Creation ITVF. It returns a table according to setted class's value
	...
	BEGIN TRY
		...
		WITH LEG_CTE (NEW_KEY, ATD, ATA, TOFF, TDWN, DIF_ATA_TDWN)
			...
			SELECT 
						NEW_KEY,
						CAST(SUBSTRING(FLTID, 4, 4) AS INT) AS ATD,
						CAST(SUBSTRING(FLTID, 4, 4) AS INT)+CAST(CAST(BLHR AS float)*60 AS INT) AS ATA,
						CAST(SUBSTRING(FLTID, 4, 4) AS INT)+20 AS TOFF,
						CAST(SUBSTRING(FLTID, 4, 4) AS INT)+20+CAST(CAST(ABHR AS float)*60 AS INT) AS TDWN,
						(CAST(SUBSTRING(FLTID, 4, 4) AS INT)+20+CAST(CAST(ABHR AS float)*60 AS INT))-(CAST(SUBSTRING(FLTID, 4, 4) AS INT)+CAST(CAST(BLHR AS float)*60 AS INT)) AS DIF_ATA_TDWN
					FROM
						LEG
			)
			...
			UPDATE LEG
				SET 
					LEG.ATD = FORMAT(DATEADD(mi,LEG_CTE.ATD,'00:00:00'), 'HH:mm'),
					LEG.TOFF = 
						FORMAT(DATEADD(mi,
							CASE 
								...
								ELSE LEG_CTE.TOFF
							END
		--has been using before 'CASE' operator
						/*
						IIF(
							SUB.DIF_ATA_TDWN>0,
							SUB.TOFF - SUB.DIF_ATA_TDWN,
							SUB.TOFF
						)
						*/
						,'00:00:00'), 'HH:mm'),
			...
			--has been using before 'CASE' operator
						...
						*/
					,'00:00:00'), 'HH:mm'),
					LEG.ATA = FORMAT(DATEADD(mi,LEG_CTE.ATA,'00:00:00'), 'HH:mm')
				FROM
					LEG_CTE
		
		--has been using before 'WITH' operator
					/*(SELECT 
						NEW_KEY,
						CAST(SUBSTRING(FLTID, 4, 4) AS INT) AS ATD,
						CAST(SUBSTRING(FLTID, 4, 4) AS INT)+CAST(CAST(BLHR AS float)*60 AS INT) AS ATA,
						CAST(CAST(BLHR AS float)*60 AS INT) AS BLHRR,
						CAST(SUBSTRING(FLTID, 4, 4) AS INT)+20 AS TOFF,
						CAST(SUBSTRING(FLTID, 4, 4) AS INT)+20+CAST(CAST(ABHR AS float)*60 AS INT) AS TDWN,
						CAST(CAST(ABHR AS float)*60 AS INT) AS ABHRR,
						(CAST(SUBSTRING(FLTID, 4, 4) AS INT)+20+CAST(CAST(ABHR AS float)*60 AS INT))-(CAST(SUBSTRING(FLTID, 4, 4) AS INT)+CAST(CAST(BLHR AS float)*60 AS INT)) AS DIF_ATA_TDWN
					FROM
						LEG) AS SUB 
