...
INSERT INTO LEG(FLTID, DATOP, LEGNO, DEPSTN, ARRSTN, CTRYCDFR, CTRYCDTO, BLHR, ABHR, GRPNO, STC_ID, AIRCRAFT_ID, NEW_KEY) 
	SELECT 
		FLTID, 
		DATOP, 
		LEGNO, 
...
GROUP BY NEW_KEY, FLTID, DATOP, LEGNO,DEPSTN, ARRSTN, CTRYCDFR, CTRYCDTO, BLHR, ABHR, GRPNO, STC, ACREG, ACVER
...
INSERT INTO LEG_PAXCAT(PAXCAT, BD, TRF, TRT, LEG_ID) 
	SELECT
	PAXCAT, 
	PARSE(BD AS int), 
	PARSE(TRF AS int), 
...
FROM RawAvia 
GROUP BY PAXCAT, BD, TRF, TRT, FLTID, DATOP, DEPSTN, ARRSTN
ORDER BY DATOP, FLTID, PAXCAT;
INSERT INTO LOGS(DATE_OF_LOGGING, LOG_DATA) SELECT GETDATE(), CAST(@@ROWCOUNT AS varchar(10))+' - '+'rows affected in LEG_CLASS';
